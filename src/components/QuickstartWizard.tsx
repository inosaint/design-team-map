...
import { DEFAULT_SETTINGS, DEFAULT_DESIGNER_TYPES } from '../types';
import type { TeamNode, DesignerTypeConfig } from '../types';
...
function generateTemplate(
  ...
  // Get the first selected type or default to 'product'
  const primaryType = selectedTypes[0] || 'product';
  ...
  if (structure === 'flat') {
    ...
    const lead = createMember('Design Lead', leadLevel, primaryType, null, 'ic');
    nodes.push(lead);
    ...
    for (let i = 0; i < memberCount; i++) {
      ...
      const member = createMember(`Designer ${i + 1}`, level, type, lead.id);
      nodes.push(member);
    ...
    const head = createMember('Head of Design', 6, primaryType, null);
    nodes.push(head);
    positions.set(head.id, { x: 400, y: 50 });

    if (size === 'tiny') {
      // Just head + 2 designers
      for (let i = 0; i < 2; i++) {
        const type = selectedTypes[i % selectedTypes.length] || primaryType;
        const member = createMember(`Designer ${i + 1}`, 3, type, head.id);
        nodes.push(member);
        positions.set(member.id, { x: 250 + i * 300, y: 200 });
      }
    } else if (size === 'small') {
      // Head -> 2 senior -> 4 designers
      const managerCount = 2;
      for (let m = 0; m < managerCount; m++) {
        const type = selectedTypes[m % selectedTypes.length] || primaryType;
        const manager = createMember(m === 0 ? 'Senior Designer' : 'Design Manager', 4, type, head.id, m === 0 ? 'ic' : 'manager');
        nodes.push(manager);
        positions.set(manager.id, { x: 200 + m * 400, y: 180 });

        for (let i = 0; i < 2; i++) {
          const memberType = selectedTypes[(m * 2 + i) % selectedTypes.length] || primaryType;
          const member = createMember(`Designer ${m * 2 + i + 1}`, 2, memberType, manager.id);
          nodes.push(member);
      ...
      for (let m = 0; m < managerCount; m++) {
        const type = selectedTypes[m % selectedTypes.length] || primaryType;
        const manager = createMember(`Design Manager ${m + 1}`, 5, type, head.id, 'manager');
        nodes.push(manager);
        ...
        for (let l = 0; l < leadCount; l++) {
          ...
          const lead = createMember(`Senior Designer ${m * leadCount + l + 1}`, 4, leadType, manager.id, 'ic');
          nodes.push(lead);
          ...
          for (let i = 0; i < icCount; i++) {
            ...
            const member = createMember(`Designer`, 2 + (i % 2), icType, lead.id);
            nodes.push(member);
      ...
      // Add planned hire
      const plannedHire = createPlannedHire('TBD - Senior', 4, secondaryType, nodes[1].id, 'ic');
      nodes.push(plannedHire);
    ...
    const podNames = ['Product', 'Platform', 'Growth', 'Enterprise'];
    ...
    for (let p = 0; p < podCount; p++) {
      ...
      // Pod lead
      const leadLevel = size === 'tiny' ? 4 : 5;
      const lead = createMember(`${podNames[p]} Lead`, leadLevel, podType, headId, size === 'tiny' ? 'ic' : 'manager');
      nodes.push(lead);
      ...
      for (let i = 0; i < memberCount; i++) {
        ...
        const level = i === 0 ? 3 : 2;
        const member = createMember(`${podNames[p]} Designer ${i + 1}`, level, memberType, lead.id);
        nodes.push(member);
...
export default function QuickstartWizard({ onClose }: QuickstartWizardProps) {
  ...
  const [selectedTypes, setSelectedTypes] = useState<string[]>(['product', 'ux', 'ui']);
  ...
  const steps = [
    ...
    { title: 'Specializations', subtitle: 'What types of designers?' },
    { title: 'Ready!', subtitle: 'Preview your team map' },
  ];
  ...
  const handleApply = () => {
    const { nodes, positions } = generateTemplate(teamSize, structureType, selectedTypes);

    // Filter designer types to only selected ones
    const filteredTypes: DesignerTypeConfig[] = DEFAULT_DESIGNER_TYPES.filter(t =>
      selectedTypes.includes(t.id)
    );

    // If no types selected, use defaults
    const typesToUse = filteredTypes.length > 0 ? filteredTypes : DEFAULT_DESIGNER_TYPES;
  ...
  const { nodes: previewNodes } = generateTemplate(teamSize, structureType, selectedTypes);
        ...
        <div className={styles.content}>
          {step === 0 && (
            <div className={styles.welcomeStep}>
              ...
              <h3 className={styles.welcomeTitle}>Build your team map in seconds</h3>
              <p className={styles.welcomeDesc}>
                Answer a few quick questions and we'll create a starter org chart
                tailored to your team's size and structure. You can customize
                everything afterwards.
              </p>
              <ul className={styles.welcomeList}>
                <li>Choose your team size</li>
                <li>Pick an organizational structure</li>
                <li>Select designer specializations</li>
                <li>Get a ready-to-customize team map</li>
              </ul>
            </div>
          )}
          ...
          {step === 3 && (
            <div className={styles.typesStep}>
              <p className={styles.typesHint}>Select the designer roles in your team</p>
              <div className={styles.typesGrid}>
                {DEFAULT_DESIGNER_TYPES.map((type) => (
                  <button
                    key={type.id}
              ...
              {selectedTypes.length === 0 && (
                <p className={styles.typesWarning}>Select at least one designer type</p>
              )}
            </div>
          )}
              ...
              <div className={styles.previewCard}>
                ...
                <div className={styles.previewStats}>
                  ...
                  <div className={styles.previewStat}>
                    ...
                    <span className={styles.previewStatLabel}>Designer types</span>
                  </div>
                </div>
              </div>
              <div className={styles.previewSummary}>
                <h4>Your team structure:</h4>
                <ul>
                  ...
                  <li><strong>Types:</strong> {selectedTypes.map(t => DEFAULT_DESIGNER_TYPES.find(dt => dt.id === t)?.abbreviation).join(', ')}</li>
                </ul>
              </div>
            </div>
          )}
        ...
        <div className={styles.footer}>
          ...
          {step < steps.length - 1 ? (
            <button
              ...
              disabled={step === 3 && selectedTypes.length === 0}
            >
